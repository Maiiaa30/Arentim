  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      <div>
        ...
        <p className="mt-2 font-sans text-sm text-muted">Atravessa as faixas para subir o prémio. Mas cuidado com os carros — retira antes de seres atropelado.</p>
      </div>

      {/* Road */}
      <div className="felt felt-rail mx-auto max-w-2xl overflow-x-auto rounded-lg p-4 sm:p-5">
        <div className="flex min-w-[560px] items-stretch gap-1.5">
          <div className="flex w-12 shrink-0 flex-col items-center justify-center rounded bg-positive-felt/30 text-2xl">🐔{step === 0 && <span className="sr-only">início</span>}</div>
          {Array.from({ length: LANES }, (_, k) => {
            const lane = k + 1;
            const here = step === lane;
            const passed = step >= lane && phase !== 'idle';
            const dead = deadLane === lane;
            return (
              <div
                key={lane}
                className={`relative flex h-20 flex-1 flex-col items-center justify-center rounded border text-center ${
                  dead ? 'border-negative bg-negative/20' : here ? 'border-gold bg-gold/15' : passed ? 'border-positive/30 bg-positive/[0.06]' : 'border-border bg-black/20'
                }`}
              >
                <span className="font-mono text-[10px] text-muted-2">{laneMult(lane, s).toFixed(2)}×</span>
                <span className="text-lg">{dead ? '💥' : here ? '🐔' : passed ? '✓' : '🚗'}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="min-h-[1.75rem] text-center">
        {phase === 'playing' ? (
          <p className="font-sans text-sm text-muted">Faixa {step} · <span className="font-mono text-gold">{mult.toFixed(2)}×</span></p>
        ) : phase === 'done' && result ? (
      ...
      {phase === 'playing' ? (
        <div className="mx-auto flex max-w-2xl gap-3">
          <Button variant="secondary" onClick={advance} disabled={busy} className="flex-1">Avançar →</Button>
