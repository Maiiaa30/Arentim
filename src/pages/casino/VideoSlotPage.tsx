    ...
    >
      <defs>
        <radialGradient id="vsGlow" cx="50%" cy="32%" r="70%">
          <stop offset="0" stopColor="#5a4416" stopOpacity="0.9" />
          <stop offset="45%" stopColor="#241a08" stopOpacity="0.7" />
...
export function VideoSlotPage() {
  ...
  return (
    <div className="animate-fade-in space-y-5">
      ...
      {/* The machine — large gilded frame with a themed backdrop behind the reels */}
      <div
        className="relative mx-auto max-w-5xl rounded-[22px] p-[3px] shadow-[0_34px_100px_rgba(0,0,0,0.65)]"
        style={{ background: `linear-gradient(155deg, #f7e4ad, ${GOLD}, #5b4824)` }}
      >
        <div className="relative overflow-hidden rounded-[19px] bg-bg p-3 sm:p-6">
          {/* Marquee */}
          <div className="relative z-10 mb-4 text-center">
            <p className="font-sans text-[10px] uppercase tracking-[0.45em] text-muted-2">Arentim Slots</p>
            <h1 className="font-display text-[34px] font-bold leading-tight sm:text-[46px]" style={{ color: GOLD, textShadow: `0 0 26px ${GOLD}77` }}>
              Fortuna de Ouro
            </h1>
          </div>

          {/* Reel frame: backdrop scene visible behind translucent 5×3 cells */}
          <div
            className="relative overflow-hidden rounded-xl"
            style={{
              aspectRatio: '5 / 3',
              boxShadow: 'inset 0 0 0 2px rgba(201,162,75,0.45), inset 0 0 50px rgba(0,0,0,0.7)',
            }}
          >
            <VaultBackdrop />
          ...
          {/* Outcome line */}
          <div className="relative z-10 mt-3 flex h-8 items-center justify-center text-center">
            {busy ? (
      ...
      {/* Control bar — SALDO · bet stepper + MAX · SPIN · paytable toggle */}
        ...
        <div className="card flex flex-col items-center gap-4 p-4 sm:flex-row sm:justify-between">
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="min-w-[84px] text-center font-mono text-lg font-bold tabular-nums text-gold-light">{formatAmount(stake)}</span>
              <button
                type="button"
              ...
              <button
                ...
                className="focus-ring rounded-full border border-border px-3 py-1.5 font-sans text-[11px] uppercase tracking-wider text-muted-2 hover:text-text disabled:opacity-40"
              >
                Máx
              </button>
            </div>
          ...
          >
            {busy ? <span className="h-8 w-8 animate-spin rounded-full border-[3px] border-bg/30 border-t-bg" /> : 'Girar'}
          </button>
        </div>
      ...
      <div className="mx-auto max-w-5xl">
        <button
          ...
          className="focus-ring mb-2 font-sans text-[10.5px] font-medium uppercase tracking-[0.18em] text-muted-2 hover:text-text"
        >
          Tabela de prémios <span className="text-faint">(× a aposta · por linha) {showPaytable ? '▲' : '▼'}</span>
        </button>
        {showPaytable && (
        ...
        <p className="mt-3 font-sans text-[11px] leading-relaxed text-muted-2">
          9 linhas fixas. Cada linha paga a maior sequência de iguais a contar da esquerda (≥3). Os prémios somam-se e dividem-se pelas 9 linhas. Cinco{' '}
